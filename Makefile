...
RG_TAGS={"Product" : "Get Information About Schools", "Service Offering" : "Get Information About Schools"}
REGION=Uk South
SERVICE_NAME=get-information-about-schools
SERVICE_SHORT=gias
...
arm-deployment: composed-variables set-azure-account
	...
	az deployment sub create --name "resourcedeploy-tsc-$(shell date +%Y%m%d%H%M%S)" \
		...
		--parameters "resourceGroupName=${RESOURCE_GROUP_NAME}" 'templateVersion=${ARM_TEMPLATE_TAG}' 'tags=${RG_TAGS}' \
...
.PHONY: development2
development2:
	$(eval include global_config/development2.sh)
...
.PHONY: test2
test2:
	$(eval include global_config/test2.sh)

.PHONY: preproduction
preproduction:
	$(eval include global_config/preproduction.sh)

.PHONY: preproduction2
preproduction2:
	$(eval include global_config/preproduction2.sh)
...
.PHONY: production2
production2:
	$(eval include global_config/production2.sh)